...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['export_img_url'] = html_entity_decode($this->url->link('catalog/product/export_image', 'token=' . $this->session->data['token'], true));
		...
		foreach ($results as $result) {
			$getImage = $this->model_catalog_product->getProductImagesBySku($result['sku']);
			if ($getImage) {
				if ( @getimagesize($getImage[0]['domain'].$getImage[0]['image'] ) ) {
					$image = $getImage[0]['domain'].$getImage[0]['image'];
				} else {
	...
	protected function getForm() {
		...
		$data['entry_image_url'] = $this->language->get('entry_image_url');
		$data['entry_status'] = $this->language->get('entry_status');
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images =$this->model_catalog_product->getProductImagesBySku($product_info['sku']);
		} else {
		...
		foreach ($product_images as $product_image) {
			...
			if (isset($this->request->post['product_image']))  {
				...
				$image = $product_image['domain'].$product_image_url;
				$thumb = $product_image['domain'].$product_image_url;
			} else if ( @getimagesize($product_image['domain'].$product_image['image'] ) ){
				$product_image_url = $product_image['image'];
				$image = $product_image['domain'].$product_image_url;
				$thumb = $product_image['domain'].$product_image_url;
			} else {
				$image = '';
	...
	protected function validateForm() {
		...
		$sort_check = [];
		foreach ($product_images as $product_image) {
			$sort_check[] = $product_image['sort_order'];
		}
		if(count(array_unique($sort_check))<count($sort_check)) {
			$this->error['warning'] = $this->language->get('error_image_sort');
		}
	...
	public function import_image() {
		...
		if(!$json){
			...
			if (!$json) {
				...
				// $json['j'] = $excel_data;
				$import_image = $this->model_catalog_product->import_image($excel_data);
	...
	public function export_image() {
		...
		if(!$error){
			...
			$image_excel = array( 0 => array( 'Product SKU', 'Image Domain', 'Image', 'Sort' ) );
			
			$all_product_images = $this->model_catalog_product->getAllProductImages();
			
			foreach($all_product_images as $all_product_image){
				$image_excel[] = array(
				$all_product_image['sku'],
				$all_product_image['domain'],
				$all_product_image['image'],
				$all_product_image['sort_order'],
				);
			}	
