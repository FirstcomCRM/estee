...
class ModelLocalisationCurrency extends Model {
	...
	public function refresh($force = false) {
		...
		if(@simplexml_load_file($url)){
			...
			$base = 0;
			// Get base
			foreach($XML->Cube->Cube->Cube as $rate){ //debug($rate); 
				if( (string)$rate["currency"] == $this->config->get("config_currency") ){
					$base = $rate["rate"];
					break;
				}
			} 
			...
			foreach($XML->Cube->Cube->Cube as $rate){ 
				$rates[(string)$rate["currency"]] = ( (float)$rate['rate'] / (float)$base );
			}
			...
			foreach ($query->rows as $result) {
				if ( isset($rates[$result['code']]) ) {
					$value = $rates[$result['code']];
			...
			$this->refresh_from_fixio(true);
		}
			
	}

	// Rename to refresh
	public function refresrefreshh_from_fixio($force = false) {
		$currency_data = array();
		...
		foreach ($query->rows as $result) {
			$response = file_get_contents('https://api.fixer.io/latest?symbols='.$result['code'].'&base='.$this->config->get('config_currency'));
			
			$json = json_decode($response, true);
			
			if ( isset($json['rates'][$result['code']]) ) {
				$value = $json['rates'][$result['code']];
				$this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '" . $value . "', date_modified = '" .  $this->db->escape(date('Y-m-d H:i:s')) . "' WHERE code = '" . $this->db->escape($result['code']) . "'");
			}
		}
	...
	public function _refresh($force = false) {
		$currency_data = array();
