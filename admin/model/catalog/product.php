...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', sku = '" . $this->db->escape($data['sku']) . "', domain = '" . $this->db->escape($product_image['domain']) . "',image = '" . $this->db->escape($product_image['image_url']) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_image WHERE sku = '" .  $this->db->escape($data['sku']) . "'");
	...
	public function getProduct($product_id) {
		...
		$query2 = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "product_pdf` WHERE product_id='".(int)$product_id."'");

		if($query2->row){
			$query->row['product_pdf_id'] = $query2->row['product_pdf_id'];
			$query->row['filename'] = $query2->row['filename'];
			$query->row['mask'] = $query2->row['mask'];
		}
		elseif($query->row){
			$query->row['product_pdf_id'] = "";
			$query->row['filename'] = "";
			$query->row['mask'] = "";
		}
	...
	public function getAllProductImages() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image ORDER BY sort_order ASC");
		return $query->rows;
	}

	public function getProductImagesBySku($sku) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE sku = '" . $this->db->escape($sku) . "' ORDER BY sort_order ASC");
	...
	public function import_image($excel_data) {
		// to truncate
		$this->db->query("TRUNCATE " . DB_PREFIX . "product_image");
		
		if (isset($excel_data)) {
			$result = [];
			foreach ($excel_data as $product_image) {
				if($this->db->query("
					INSERT INTO " . DB_PREFIX . "product_image 
					SET 
					sku = '" . $this->db->escape($product_image['product-sku']) . "', 
					domain = '" . $this->db->escape($product_image['image-domain']) . "', 
					image = '" . $this->db->escape($product_image['image']) . "', 
					sort_order = '" . (int)$product_image['sort'] . "'")){
					$result[] = true;
				} else {
					$result[] = false;
				}
